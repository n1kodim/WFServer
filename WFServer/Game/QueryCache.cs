...
namespace WFServer.Game
{
    ...
    public static class QueryCache
    {
        ...
        static Cache Load(string name, string data = null, string hash = null)
        {
            ...
            {
                data = File.ReadAllText("GameData/cache/" + name + ".xml");
            }
            if (string.IsNullOrEmpty(hash))
            {
                hash = Utils.GetHash(data).ToString();
            }
        ...
        static void ValidateFiles(object source, ElapsedEventArgs e)
        {
            ...
            {
                string data = File.ReadAllText("GameData/cache/" + name + ".xml");
                string hash = Utils.GetHash(data).ToString();
